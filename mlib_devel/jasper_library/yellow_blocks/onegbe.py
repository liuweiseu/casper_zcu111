...
class onegbe(YellowBlock):
    ...
    def factory(blk, plat, hdl_root=None):
        if plat.name in ['vcu118']:
            ...
            return onegbe_casia_k7(blk, plat, hdl_root)
        elif plat.name in ['zcu111']:
            return onegbe_zcu111(blk, plat, hdl_root)
        else:
...
class onegbe_zcu111(onegbe):
    def initialize(self):
        self.typecode = TYPECODE_ETHCORE
        self.add_source('onegbe/*.v')
        self.add_source('onegbe/rfsoc/*.v')
        self.add_source('onegbe/rfsoc/*.vhd')
        self.add_source('onegbe/rfsoc/*.xci')
        self.add_source('onegbe/rfsoc/*.coe')
    ...
    def modify_top(self,top):
        ...
        self._instantiate_mac(top)
        self._instantiate_phy(top)
        self._instantiate_wbToaxi4lite(top)
    ...
    def _instantiate_udp(self, top):
        ...
        top.add_axi4lite_interface(regname=self.unique_name, mode='rw', nbytes=65536, typecode=self.typecode, axi4lite_mode='raw')
        # add wb ports
        gbe_udp.add_port('wb_clk_i','axil_clk')
        gbe_udp.add_port('wb_rst_i',self.fullname + 'wb_rst')#
        gbe_udp.add_port('wb_stb_i', self.fullname + '_wb_stb')#
        gbe_udp.add_port('wb_cyc_i', self.fullname + '_wb_cyc')#
        gbe_udp.add_port('wb_we_i' , self.fullname + '_wb_we')#
        gbe_udp.add_port('wb_adr_i' , self.fullname + 'wb_adr', width=32)#
        gbe_udp.add_port('wb_dat_i' , self.fullname + 'wb_dat_i', width=32)#
        gbe_udp.add_port('wb_sel_i' , self.fullname + 'wb_sel', width=4)#
        gbe_udp.add_port('wb_dat_o' , self.fullname + 'wb_dat_o', width=32)#
        gbe_udp.add_port('wb_err_o' , self.fullname + 'wb_err')
        gbe_udp.add_port('wb_ack_o' , self.fullname + 'wb_ack')#
    ...
    def _instantiate_phy(self, top):
        gbe_pcs = top.get_instance(entity='gig_ethernet_pcs_pma_autonegotiation_1000BASEX', name=self.fullname+'_pcs_pma')
        ...
        gbe_pcs.add_port('gtrefclk_p', self.fullname+'_mgt_clk_p', dir='in', parent_port=True)
        gbe_pcs.add_port('gtrefclk_n', self.fullname+'_mgt_clk_n', dir='in', parent_port=True)
        gbe_pcs.add_port('independent_clock_bufg', 'clk_200',parent_sig=False)
        gbe_pcs.add_port('configuration_vector', '5\'b10000', parent_sig=False)
        gbe_pcs.add_port('an_adv_config_vector', '16\'b1001100000000001', parent_sig=False)
        gbe_pcs.add_port('an_restart_config', '1\'b0', parent_sig=False)
        gbe_pcs.add_port('reset', 'sys_rst')
        ...
        gbe_pcs.add_port('rxuserclk_out', 'gbe_rxuserclk_out')  # first instance only (shared logic lives in first core)
        gbe_pcs.add_port('rxuserclk2_out','gbe_rxuserclk2_out') # first instance only (shared logic lives in first core)
        gbe_pcs.add_port('gtpowergood','gbe_gtpowergood')
        gbe_pcs.add_port('resetdone', self.fullname+'_reset_done')
        gbe_pcs.add_port('pma_reset_out', '')
        gbe_pcs.add_port('mmcm_locked_out', 'gbe_mmcm_locked_out')
        gbe_pcs.add_port('gmii_isolate', '')
    ...
    def _instantiate_wbToaxi4lite(self,top):
    ...
    def gen_constraints(self):
        ...
        consts += [PortConstraint(self.fullname+'_sfp_rx_p', 'mgt_rx_p')]
        consts += [PortConstraint(self.fullname+'_sfp_rx_n', 'mgt_rx_n')]
        consts += [PortConstraint(self.fullname+'_mgt_clk_p', 'eth_clk_p')]
        consts += [PortConstraint(self.fullname+'_mgt_clk_n', 'eth_clk_n')]
