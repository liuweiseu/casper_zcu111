...
for i=1:2^(sys_config-1)
    %add adc_sync ports
    dac_di_gw_out_name = clear_name([gcb, '_', 'dac',num2str(i-1),'_','din']);
    dac_do_inport_name = clear_name(['dac',num2str(i-1),'_out','_sim']);
    dac_di_outport_name = clear_name(['dac',num2str(i-1),'_din']);
    
    %add a outport for dac
    add_block('built-in/inport', [blk,'/',dac_di_outport_name],...
              'Name', dac_di_outport_name,...
              'Position',[curr_x curr_y+100+i,...
                          curr_x+30 curr_y+116+i])
    curr_x = curr_x + 200;
    %add a gateway out for dac
    add_block('xbsIndex_r4/Gateway Out',[blk,'/',dac_di_gw_out_name],...
              'Name',dac_di_gw_out_name,...
              'Position',[curr_x curr_y+100+i,...
                          curr_x+60 curr_y+116+i])
    curr_x = curr_x + 200;
    %add a inport for dac
    add_block('built-in/outport', [blk,'/',dac_do_inport_name],...
              'Name', dac_do_inport_name,...
              'Position',[curr_x curr_y+100+i,...
                          curr_x+30 curr_y+116+i])
    curr_x = curr_x - 400;
    curr_y = curr_y + 60;
    %connect them
    add_line(blk, [dac_di_outport_name, '/1'],...
                  [dac_di_gw_out_name, '/1']);
    add_line(blk, [dac_di_gw_out_name, '/1'],...
                  [dac_do_inport_name, '/1']); 
    %add valid port
    dac_valid_gw_out_name = clear_name([gcb, '_', 'dac',num2str(i-1),'_','valid']);
    dac_valid_inport_name = clear_name(['dac',num2str(i-1),'_valid_out','_sim']);
    dac_valid_outport_name = clear_name(['dac',num2str(i-1),'_valid_in']);
    
    %add a outport for dac
    add_block('built-in/inport', [blk,'/',dac_valid_outport_name],...
              'Name', dac_valid_outport_name,...
              'Position',[curr_x curr_y+100+i,...
                          curr_x+30 curr_y+116+i])
    curr_x = curr_x + 200;
    %add a gateway out for dac
    add_block('xbsIndex_r4/Gateway Out',[blk,'/',dac_valid_gw_out_name],...
              'Name',dac_valid_gw_out_name,...
              'Position',[curr_x curr_y+100+i,...
                          curr_x+60 curr_y+116+i])
    curr_x = curr_x + 200;
    %add a inport for dac
    add_block('built-in/outport', [blk,'/',dac_valid_inport_name],...
              'Name', dac_valid_inport_name,...
              'Position',[curr_x curr_y+100+i,...
                          curr_x+30 curr_y+116+i])
    curr_x = curr_x - 400;
    curr_y = curr_y + 60;
    %connect them
    add_line(blk, [dac_valid_outport_name, '/1'],...
                  [dac_valid_gw_out_name, '/1']);
    add_line(blk, [dac_valid_gw_out_name, '/1'],...
                  [dac_valid_inport_name, '/1']); 
end
